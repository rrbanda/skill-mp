...
class ExtractedEntities(BaseModel):
    ...
    inputs: list[str] = Field(
        description="Artifacts the skill expects as input (e.g. terraform .tf files, Dockerfile, OpenAPI spec)."
    )
    outputs: list[str] = Field(
        description="Artifacts the skill produces (e.g. security report, optimized pipeline, linted markdown)."
    )
    domain: str = Field(
        description="High-level domain category (e.g. infrastructure-security, data-services, ci-cd, documentation)."
    )
...
class ClassifiedRelationship(BaseModel):
    ...
    relationship: RelationshipType = Field(description="The type of relationship between the two skills.")
    confidence: float = Field(ge=0.0, le=1.0, description="Confidence score for this classification (0.0 to 1.0).")
    direction: Literal["A_TO_B", "B_TO_A", "BIDIRECTIONAL"] = Field(description="Direction of the relationship.")
    description: str = Field(description="Human-readable explanation of why this relationship exists.")
...
class CommunitySummary(BaseModel):
    ...
    description: str = Field(description="2-3 sentence summary of what this community covers.")
    key_technologies: list[str] = Field(description="Top 3-5 technologies shared by community members.")
    member_count: int = Field(description="Number of skills in this community.")
    coherence_score: float = Field(
        ge=0.0, le=1.0, description="How well the members fit together (0.0 = loose, 1.0 = tight)."
    )
...
class BaselineComparison(BaseModel):
    ...
    assessment: str = Field(description="Brief assessment of divergence from deterministic baseline.")
...
class GraphQualityReport(BaseModel):
    ...
    total_nodes: int = 0
    total_edges: int = 0
    isolated_nodes: list[str] = Field(default_factory=list, description="Skill IDs with zero relationships.")
    communities_detected: int = 0
    avg_confidence: float = 0.0
    low_confidence_edges: int = Field(default=0, description="Edges with confidence < 0.8.")
    edges_by_type: dict[str, int] = Field(default_factory=dict, description="Count of edges per relationship type.")
    baseline_comparison: BaselineComparison | None = Field(
        default=None, description="Comparison with deterministic baseline, if available."
    )
    issues: list[str] = Field(default_factory=list, description="Specific quality issues found.")
    recommendations: list[str] = Field(default_factory=list, description="Suggestions for improving graph quality.")
